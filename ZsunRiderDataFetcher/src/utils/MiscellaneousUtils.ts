 ...
 */
export function isDictionaryOfObjects(obj: any): obj is Record<string, Record<string, any>> {
 ...
 */
export function isEmptyDictionary(obj: any): obj is Record<string, never> {
    return obj !== null &&
        typeof obj === "object" &&
        !Array.isArray(obj) &&
 ...
 *   dictionaryToArray({}); // []
 */
export function dictionaryToArray<T>(dict: { [key: string]: T } | null | undefined): T[] {
...
export async function retryAsync<T>(
  ...
  attempts = 3,
  delayMs = 500,
  shouldRetry: (err: any, attempt: number) => boolean = () => true
): Promise<T> {
  let lastError: any;
 ...
 */
export function formatIsoDateNoMs(date: Date): string {
