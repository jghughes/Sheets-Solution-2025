function writeSourceDataSheet(records, nameOfSheet) {
    ...
    if (!sheet) sheet = ss.insertSheet(nameOfSheet); else sheet.clear();
    ...
    for (var i = 0; i < records.length; i++) {
        var r = records[i] || {};
        for (var k in r) {
            if (Object.prototype.hasOwnProperty.call(r, k)) keySet[k] = true;
        }
    }

    var keys = [];
    for (var k2 in keySet) { if (k2 !== "zwiftId") keys.push(k2); }
    keys.sort();
