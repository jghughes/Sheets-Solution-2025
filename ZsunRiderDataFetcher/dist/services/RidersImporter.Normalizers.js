...
function dictionaryToArray(dict) {
    ...
    for (var key in dict) {
        ...
        var entry = {};
        for (var k2 in raw) {
            if (!Object.prototype.hasOwnProperty.call(raw, k2))
                continue;
            entry[k2] = raw[k2];
            try {
...
function dictionaryToPrecomputedArray(normalizedRiders) {
    if (!normalizedRiders || !Array.isArray(normalizedRiders))
        return [];
    var out = [];
...
function makeRiderInitials(rider) {
    if (!rider || !rider.name || typeof rider.name !== "string")
        return "";
...
//# sourceMappingURL=RidersImporter.Normalizers.js.map