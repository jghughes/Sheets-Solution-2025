...
const fetchPlainTextFileFromMyDrive = (filename, opName = "MyDriveFetch", driveAppImpl = (typeof DriveApp !== "undefined" ? DriveApp : null)) => {
    ...
    if (!driveApp)
        throw new ServerError(`${opName}_unavailable`, `${opName} unavailable: no DriveApp`);
    try {
 ...
 */
const fetchPlainTextFileFromSharedLinkToGoogleDrive = (sharedLink, opName = "GoogleDriveFetch", driveAppImpl = (typeof DriveApp !== "undefined" ? DriveApp : null), coreUtil = (typeof SheetsDataFetcherCore !== "undefined" ? SheetsDataFetcherCore : null)) => {
    const driveApp = driveAppImpl;
    const util = coreUtil;
    if (!driveApp || !util)
        throw new ServerError(`${opName}_unavailable`, `${opName} unavailable: missing DriveApp or core util`);
    try {
...
const fetchPlainTextFileFromUrl = (url, opName = "HttpFetch", fetchImpl = (typeof UrlFetchApp !== "undefined" ? UrlFetchApp : null)) => {
    const fetcher = fetchImpl;
    if (!fetcher)
        throw new ServerError(`${opName}_unavailable`, `${opName} unavailable: no fetch implementation`);
    try {
        const resp = (typeof fetcher.fetch === "function")
